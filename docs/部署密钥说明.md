...
| **NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID** | WalletConnect 连接（Web 端扫码等） | 在 [WalletConnect Cloud](https://cloud.walletconnect.com) 创建项目，复制 Project ID | `.env`，web 服务（构建时通过 build args 注入） | 可选（用 WalletConnect 时必填） |
